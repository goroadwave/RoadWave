...
export default function TourPage() {
  ...
  function toggleSlide(id: SlideId) {
    setEnabledIds((prev) => {
      ...
      if (nextSet.has(id)) {
        if (nextSet.size === 1) return prev // never let them disable everything
        nextSet.delete(id)
      } else {
        nextSet.add(id)
      }
    ...
    // After a toggle, clamp index to the new shorter list.
  ...
  return (
    <main className="min-h-screen bg-night text-cream font-sans flex flex-col">
      ...
      <section className="flex-1 px-4 pb-4">
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 inline-flex items-center gap-1 rounded-full border border-flame/40 bg-night px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-flame">
              Riley · Host
            </div>
          </div>
      ...
      {/* Footer nav */}
      <div className="p-4 pb-6 max-w-md w-full mx-auto flex items-center gap-3">
        <button
          type="button"
          onClick={prev}
...
function CtaRow() {
  return (
    <div className="flex flex-col sm:flex-row gap-2 flex-1 sm:flex-initial">
      <Link
        href="/"
        className="inline-flex items-center justify-center gap-2 rounded-xl bg-flame text-night px-4 py-2.5 text-sm font-semibold shadow-lg shadow-flame/15 hover:bg-amber-400 transition-colors"
      >
        Get Started <span aria-hidden>👋</span>
      </Link>
      <Link
        href="/demo"
        className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 text-cream px-4 py-2.5 text-sm font-semibold hover:bg-white/10 hover:border-flame/40 transition-colors"
      >
        Try the Demo →
      </Link>
    </div>
